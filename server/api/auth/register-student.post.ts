...
import { registerStudentSchema } from '../../utils/validate'
...
export default defineEventHandler(async (event) => {
  ...
  const parsed = registerStudentSchema.safeParse(upload.fields)
  if (!parsed.success) {
  ...
  await query(
    `INSERT INTO pending_registrations (token, role, email, payload, code_hash, expires_at)
     VALUES (?, 'student', ?, ?, ?, DATE_ADD(NOW(), INTERVAL 15 MINUTE))`,
  ...
  return { ok: true, token, mode: 'register' as const, role: 'student' as const }
})
